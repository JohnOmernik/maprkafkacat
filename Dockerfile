...
RUN ldconfig && wget https://github.com/edenhill/kafkacat/archive/$CATVER.tar.gz && tar zxf $CATVER.tar.gz
RUN  cd kafkacat-$CATVER && ./configure --libdir=/opt/mapr/lib --includedir=/opt/mapr/include && make && make install && ldconfig && cd .. && rm -rf kafkacat
