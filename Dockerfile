...
RUN wget https://github.com/edenhill/kafkacat/archive/$CATVER.tar.gz && tar zxf $CATVER.tar.gz && cd kafkacat-$CATVER && ./configure --libdir=/opt/mapr/lib --includedir=/opt/mapr/include/librdkafka &&  make --CFLAGS="-Wall -I/opt/mapr/include/librdkafka -g -std=c99" && make install && ldconfig && cd .. && rm -rf kafkacat-$CATVER && rm $CATVER.tar.gz
